...
Compatibility fixtures are listed in
[Schemas And Fixtures](reference/schemas.md#compatibility-fixtures). The test suite validates the
exact `1.0.0` fixtures against these schemas and checks the runtime compatibility policy for legacy,
same-major, and next-major manifest fixtures.
