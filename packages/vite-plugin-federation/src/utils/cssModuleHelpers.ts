...
import type { OutputBundleItemLike, OutputBundleLike } from './outputBundle';

export type OutputBundleItem = OutputBundleItemLike;
 ...
 */
export const collectCssAssets = (bundle: OutputBundleLike): Set<string> => {
...
export const processModuleAssets = (
  ...
  for (const [fileName, fileData] of Object.entries(bundle)) {
    ...
    for (const modulePath of Object.keys(fileData.modules)) {
      ...
      const importedCss = fileData.viteMetadata?.importedCss;
      const importedCssList = importedCss ? Array.from(importedCss) : [];
      if (importedCssList.length > 0) {
        for (const cssFile of importedCssList) {
